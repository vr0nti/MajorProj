...
exports.addClass = async (req, res) => {
  try {
    ...
    // Always use the department from the authenticated admin to avoid mismatch issues
    const targetDepartmentId = req.user.department;

    // If the client sends a different departmentId, ignore it but warn (does not block)
    if (departmentId && departmentId.toString() !== targetDepartmentId.toString()) {
      console.warn(`DepartmentAdmin attempted to create class for other department. Ignored. userDept=${targetDepartmentId}, bodyDept=${departmentId}`);
    }
...
exports.updateClass = async (req, res) => {
  try {
    ...
    if (String(classData.department) !== String(req.user.department)) {
